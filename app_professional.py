...
<style>
    ...
    .main, .stApp, .block-container, .element-container {{
        ...
        color: {ORA_COLORS['primary_dark']} !important;
    }}
    ...
    .data-table th {{
        background: {ORA_COLORS['professional_blue']};
        color: white;
        font-weight: 600;
        padding: 12px;
        text-align: left;
    ...
    .subsection-header {{
        color: {ORA_COLORS['professional_blue']};
        font-family: 'Inter', sans-serif;
        font-weight: 600;
        font-size: 1.1rem;
    ...
    /* Buttons */
    .stButton > button {{
        background: linear-gradient(135deg, {ORA_COLORS['professional_blue']}, {ORA_COLORS['primary_cyan']});
        color: white;
        border: none;
        border-radius: 6px;
        font-family: 'Inter', sans-serif;
    ...
    .stSelectbox > div > div,
    .stSlider {{
        border-color: {ORA_COLORS['professional_blue']};
        color: {ORA_COLORS['primary_dark']} !important;
        background-color: {ORA_COLORS['white']} !important;
    }}
...
if logo_base64:
    st.markdown(f"""
    <div style="display: flex; align-items: center; margin-bottom: 1rem; padding: 1rem 0; border-bottom: 2px solid {ORA_COLORS['primary_cyan']};">
        <img src="data:image/png;base64,{logo_base64}" width="80" style="margin-right: 1.5rem;">
        <div>
            <div style="font-size: 2.5rem; color: {ORA_COLORS['primary_dark']}; font-family: 'Inter', sans-serif; font-weight: 700; margin: 0;">ORA LIVING</div>
            <div style="color: {ORA_COLORS['teal']}; font-family: 'Inter', sans-serif; font-size: 1.1rem; margin-top: 0.2rem;">Professional Multi-State Financial Model</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
else:
    st.markdown(f"""
    <div style="margin-bottom: 1rem; padding: 1rem 0; border-bottom: 2px solid {ORA_COLORS['primary_cyan']};">
        <div style="font-size: 2.5rem; color: {ORA_COLORS['primary_dark']}; font-family: 'Inter', sans-serif; font-weight: 700; margin: 0;">🏥 ORA LIVING</div>
        <div style="color: {ORA_COLORS['teal']}; font-family: 'Inter', sans-serif; font-size: 1.1rem; margin-top: 0.5rem;">Professional Multi-State Financial Model</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for code in ["99458", "99439", "99427"]:
        if code in sc["rates"]:
            sc["rates"][code]["multiplier"] = st.slider(
                ...
                0.1,
                help=f"Population average billing frequency for {code}"
            )

# Main content tabs
tabs = st.tabs(["🏠 **Dashboard**", "📊 **Data Tables**", "📈 **Analysis & Charts**", "⚙️ **Advanced Settings**"])
...
with tabs[2]:  # Charts & Analysis
    ...
    if "multistate_df" in st.session_state and not st.session_state.multistate_df.empty:
        ...
        # Chart selector
        chart_options = ["Revenue Stacked Bar", "Patient Growth", "Financial Dashboard", "State Comparison"]
